...
const int Lz=1000;
//--------
const double Epsilon0=1, Mu0=15;
...
vector3D LatticeBoltzmann::feq(vector3D & D,vector3D & B,int r,int i,
  ...
  vector3D Aux;
  if(r == 0){
    Aux = D*exp(-sigma0/(epsr*Epsilon0)) - 3*((v[i]^B)*1/(mur0*Mu0));
  }
  if(r == 1){
  ...
  for(ix=0;ix<Lx;ix++) //para cada celda
    for(iy=0;iy<Ly;iy++)
      for(iz=0;iz<Lz;iz++){
        ...
        for(r=0; r < 2; r++)
          for(i=0;i < n; i++)
            fnew[ix][iy][iz][r][i]=f[ix][iy][iz][r][i]=feq(D0,B0,r,i,epsilonr0,mur0,sigma0);
      }
}
    ...
    for(ix=0;ix<Lx;ix++) //para cada celda
        for(iy=0;iy<Ly;iy++)
            for(iz=0;iz<Lz;iz++){
                ...
                for(r = 0; r < 2; r++)
                  for(i=0; i < n; i++)
                    fnew[ix][iy][iz][r][i]=2*feq(D0,B0,r,i,epsilonr0,mur0,sigma0)-f[ix][iy][iz][r][i];
...
void LatticeBoltzmann::Print(void){
  ...
  for(iz=Lz/4.0;iz<Lz;iz++){
    //Compute the electromagnetic constants
    sigma0=sigma(ix,iy,iz); mur0=mur(ix,iy,iz); epsilonr0=epsilonr(ix,iy,iz);
    //Compute the Fields
    D0=D(ix,iy,iz,true)*exp(sigma0/(epsilonr0*Epsilon0));
    B0=B(ix,iy,iz,true);
    E0=E(D0,epsilonr0);
    //Print
