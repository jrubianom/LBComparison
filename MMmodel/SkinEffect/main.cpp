...
const double T = 17.68*Lz/1000.0/C*sqrt(epsilonr(0,0,0)*mur(0,0,0)), omega = 2*M_PI/T;
const double Sigma0=0.1*Epsilon0*epsilonr(0,0,0)/T;
...
void LatticeBoltzmann::ImposeFields(int t){
  ...
  iz=Lz-1; //On the whole incident plane
  for(ix=0;ix<Lx;ix++) //for each cell
    for(iy=0;iy<Ly;iy++){
      //Compute the constants
      sigma0=sigma(ix,iy,iz); mur0=mur(ix,iy,iz); epsilonr0=epsilonr(ix,iy,iz);
...
void LatticeBoltzmann::Print(vector<double>& EAmplit){
  ...
  int iAmpl = 0;
  ofstream Myfile("Results/data.dat");
  double delta, mu,aux;
    for(iz=0;iz<Lz;iz++){
