...
double ratio_eps(const double* arrEps){
  double rEt = 1,r;
...
double LatticeBoltzmann::epsilonr(int ix,int iy, int iz){
  ...
  //double Zs[2] = {z0,static_cast<double>(Lz)};
  epsr = epsrs[0];
...
void LatticeBoltzmann::Print(ofstream &fileAmpl, ofstream &fileError,int Ref){
  ...
  Er = Er/E00; Et = Et/E00;
  double Ettheo = ratio_eps(epsrs);
  fileError << Ref << "\t" << Et << "\t" << Ettheo << "\t" << abs(100*(Ettheo - Et)/Ettheo) << endl;
  cout << Ref << "\t" << Et << "\t" << Ettheo << "\t" << abs(100*(Ettheo - Et)/Ettheo) << endl;
}
