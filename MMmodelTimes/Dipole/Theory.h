...
class Theo{
        ...
        public:
                ...
                double B(double x, double z, double t);
...
double Theo::B(double x, double z, double t){
        // Magnetic field at zx plane. It only has y-component
        double r = sqrt(x*x + z*z);
        double rxp = x*p;
        double A = k*k*Z0*rxp/(4*M_PI);
        return A/(r*r)*(cos(k*r-omega*t)-sin(k*r-omega*t)/(k*r)); 
}
...
double Theo::Power(double theta,double phi,double R){
        ...
        double Ae = C*Ab;
        return Ab/Mu0*Ae*pow(sin(theta),2)*0.5;
}