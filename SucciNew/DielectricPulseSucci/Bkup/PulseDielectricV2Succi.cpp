...
vector3D LatticeBoltzmann::E(int ix,int iy,int iz,bool UseNew){
  ...
  vector3D Eaux;
  double Evec[3];
  double c_sqrd = c2Prime(ix,iy,iz);
  for(int gamma=0; gamma < 3; gamma++){
    sum = 0;
  ...
  for(ix=0;ix<Lx;ix++) //para cada celda
    for(iy=0;iy<Ly;iy++)
      for(iz=0;iz<Lz;iz++){
        ...
        c_sqrd = c2Prime(ix,iy,iz);
        speedCell = sqrt(c2(ix,iy,iz));
        B0.cargue(0,(E00/speedCell/Xfactor)*exp(-pow(iz-iz0,2)/(2*pow(alpha0,2))),0);
...
void LatticeBoltzmann::Print(void){
  ...
  vector3D B0,E0;
  ofstream Myfile("data.dat");
  double Et, Er,EAmp,speedCell;
  for(iz=0;iz<Lz;iz++){
    //Compute the Fields
    E0=E(ix,iy,iz,true); B0=B(ix,iy,iz,true)*Xfactor;
    speedCell = sqrt(c2(ix,iy,iz));
    //cout<<iz<<" "<<0.5*(eps*E2+B2/mus)<<endl;
    Myfile<<double(iz)/Lz<<" "<<E0.x()/E00<< " "<< B0.y()/(E00/speedCell) <<endl;
