...
const int Lx=1;
const int Ly=1;
const int Lz=100*scale;
///-----------
const double Tau = 0.5;
const double UTau = 1/Tau;
const double UmUTau=1-1/Tau;
//------------
//const double Epsilon0= 1/scale, Mu0 = 2*scale;
...
double epsilonr(int ix,int iy, int iz){
  return (epsr1 + epsr2)/2.0 + (epsr2 - epsr1)/2*tanh(interf*(iz-Lz/2));
}
...
void LatticeBoltzmann::Start(void){
  ...
  double alpha0=Lz*0.5/(10*sqrt(2)),iz0=Lz/2-Lz/6.0;
...
void LatticeBoltzmann::Print(void){
  ...
  for(iz=0;iz<Lz;iz++){
    ...
    Myfile<<double(iz)/Lz<<" "<<Eprima0.x()/E00<<endl;
    EAmp = abs(Eprima0.x());
