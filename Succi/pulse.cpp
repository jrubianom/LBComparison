...
LB::LB(void){
    ...
    g.resize(Lx*Ly*Lz*(2*D+1)*9);
    gnew.resize(Lx*Ly*Lz*(2*D+1)*9);
...
double LB::weight_i(int ix, int iy, int iz){
    ...
    iz : z index of the cell
    */
    return (1.0 - weight_0(ix, iy, iz))/(2*D);
}
// Macroscopic tensors
                ...
                for(int a=0; a<3; a++)
                    for(int b=0; b<3; b++)
                        for(int i=0; i<2*D+1; i++)
                        {
                            ...
                            // BGK Evolution
                            gnew[idx]=(1.0-1.0/tau)*g0 + (1.0/tau)*g_equi + T0*Dt;
                            }
}
void LB::ImposeFields(int t){
    ...
    */
    ...
    double amp = 2.0, alpha=0.01; int ix0=40;
                    ...
                    for(int a=0; a<3; a++)
                        for(int b=0; b<3; b++)
                            for(int i=0; i<2*D+1; i++){
                                // Electric field
                                // Only y component
                                double Ey = amp*std::exp(-alpha*(ix-ix0)*(ix-ix0));
                                if(a==2 && b==0){
                                    lambda_12 = -Ey;
                                } else if(a==0 && b==2){
                                    lambda_12 = Ey;
                                ...
                                // Impose field
                                int idx = index(ix, iy, iz, a, b, i);
                                gnew[idx] = geq(ix, iy, iz, a, b, i, lambda_12, B_1, B_2);
                            }
                ...
                for(int a=0; a<3; a++)
                    for(int b=0; b<3; b++)
                        for(int i=0; i<2*D+1; i++){
                            ...
                            int ixnew=(ix+v[i][0]+Lx); ixnew = ixnew%Lx;
                            int iynew=(iy+v[i][1]+Ly); iynew = iynew%Ly;
                            int iznew=(iz+v[i][2]+Lz); iznew = iznew%Lz;
                            
                            int idx_old=index(ix, iy, iz, a, b, i);
                            int idx_new=index(ixnew, iynew, iznew, a, b, i);
                            
                            g[idx_new]=gnew[idx_old];
                }
}
void LB::Print(int t){
    // Names
    ...
    for(int iz=0; iz<Lz; iz++)
        for(int iy=0; iy<Ly; iy++)
            for(int ix=0; ix<Lx; ix++){
                // Electric field
                double Ex = -(1.0/D)*Lambda(ix, iy, iz, 1, 2, true);
                double Ey = -(1.0/D)*Lambda(ix, iy, iz, 2, 0, true);
                double Ez = -(1.0/D)*Lambda(ix, iy, iz, 0, 1, true);
                // Magnetic field
