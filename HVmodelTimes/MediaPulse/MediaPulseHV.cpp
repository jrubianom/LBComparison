...
class LatticeBoltzmann{
  ...
  public:
    ...
    void Advection(void);
    void Print(ofstream &fileAmpl,ofstream &fileError,int i,double time);
};
...
void LatticeBoltzmann::Print(ofstream &fileAmpl, ofstream &fileError,int i,double time){
  ...
  for(iz=0;iz<Lz;iz++){
    ...
    fileAmpl<<i<<" "<<double(iz)/Lz<<" "<<E0.x()/E00<<endl;
  ...
  fileError << i << "\t" << time << "\t" << Er << "\t"  << Ertheo << "\t" <<  abs(100*(Ertheo - Er)/Ertheo) << "\t" <<
...
int main(){
  ...
  MyError << "Refinement\tTime\tSimulated Er \tTheorical Er \tRelative Error\t"<<
  ...
  for (int i = 1; i < 11; i++){
    ...
    // Print fields to plot their profiles
    ...
    for (int iteration=0; iteration<10; iteration++){
      ...
      DielectricPulse.Print(Myfile,MyError,i,ellapsed);
    }
    Myfile.close();
  }
  MyError.close();
