...
void get_memory(double N_ref){
  ofstream MyMem("Datos/Memory_Dipole_Miller.dat");
  MyMem << "Refinement\tMemory Used\n";
  for(double i=0.5; i < N_ref; i+=0.5){
    int f_size = Qr*Qp*Qi*Qj*Lx*Ly*Lz*pow(i,3);
    int f0_size = Lx*Ly*Lz*pow(i,3);
    MyMem << i << "\t" << sizeof(double)*2*(f_size + f0_size)/1000000 << endl;
  }
}
...
int main(){
  ...
  //Print Memory used for differente refinements
  get_memory(3.0);
