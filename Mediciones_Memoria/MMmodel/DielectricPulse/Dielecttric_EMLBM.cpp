...
//-------------------
  ...
  //Tamaño funciones de distribucion
  fileMem << Ref << "\t" << sizeof(double)*(2*Lx*Ly*Lz*Qr*Qp*Qi*Qj + 2*Lx*Ly*Lz)/1000 << endl;
    }
...
int main(){
  ...
  MemFileName = "Memory_Pulse_Miller.dat";
  ofstream MyError("Errors.dat");
  ofstream MyMem(MemFileName);
  MyError << "Refinement\tSimulated Er \tTheorical Er \tRelative Error\t"<<
