...
double epsilonr(int ix,int iy, int iz){
    return 1;//1.75+0.75*tanh((double)(iz-(Lz/2.0)));
}
...
void LatticeBoltzmann::Start(void){
  ...
  B0.cargue(0,0*B00*exp(-0.25*pow(iz-iz0,2)/(alpha0*alpha0)),0);
  Eprima0.cargue(0*E00*exp(-0.25*pow(iz-iz0,2)/(alpha0*alpha0)),0,0);
...
void LatticeBoltzmann::ImposeFields(int t){
  ...
  for(ix=0;ix<Lx;ix++) //para cada celda
    for(iy=0;iy<Ly;iy++){
        ...
        Eprima0.cargue(0*E00*sin(omega*t),0,0);
        D0=E0*epsilonr0; H0=H(B0,mur0);
        //Impose f=fnew=feq with the desired fields
        f0new[ix][iy][iz]=f0[ix][iy][iz]=feq0(rhoc0);
        for(r=0;r<2;r++)
